namespace Vpiska.Api.Control.Http

open System
open System.IO
open FSharp.Control.Tasks
open Microsoft.AspNetCore.Http
open Microsoft.AspNetCore.Mvc
open Vpiska.Domain.Event
open Vpiska.Domain.Event.Commands

[<CLIMutable>]
type AddMediaArgs =
    { EventId: string
      Media: IFormFile }
    member this.toCommand (ownerId: string) (imageId: string) =
        task {
            match Object.ReferenceEquals(this.Media, null) with
            | true -> return { EventId = this.EventId; OwnerId = ownerId
                               MediaData = null; ContentType = null; ImageId = imageId } |> EventCommand.AddMedia
            | false ->
                use stream = new MemoryStream()
                do! this.Media.CopyToAsync stream
                return { EventId = this.EventId; OwnerId = ownerId; MediaData = stream.ToArray()
                         ContentType = this.Media.ContentType; ImageId = imageId } |> EventCommand.AddMedia
        }

[<CLIMutable>]
type RemoveMediaArgs =
    { EventId: string
      MediaLink: string }
    member this.toCommand (ownerId: string) =
        { EventId = this.EventId; OwnerId = ownerId; MediaLink = this.MediaLink } |> EventCommand.RemoveMedia

[<CLIMutable>]
type CreateEventArgs =
    { Name: string
      Coordinates: string
      Address: string
      Area: string }
    member this.toCommand (eventId: string) (ownerId: string) =
        { EventId = eventId; OwnerId = ownerId; Name = this.Name
          Coordinates = this.Coordinates; Address = this.Address
          Area = this.Area } |> EventCommand.CreateEvent
    
[<CLIMutable>]    
type CloseEventArgs =
    { EventId: string }
    member this.toCommand (ownerId: string) = { EventId = this.EventId; OwnerId = ownerId } |> EventCommand.CloseEvent

module EventHttpResponse =
    
    let private createErrorResult (error: AppError) = { ErrorCode = Errors.mapAppError error }
    
    let private mapErrors (errors: AppError[]) =
        ObjectResult({ IsSuccess = false; Result = null; Errors = errors |> Array.map createErrorResult }, StatusCode = 200)
        
    let private mapEvent (domainEvent: DomainEvent) =
        match domainEvent with
        | EventCreated args -> ObjectResult(HttpResponse.createValueResult args, StatusCode = 200)
        | EventClosed -> ObjectResult(HttpResponse.createResult(), StatusCode = 200)
        | MediaAdded args -> ObjectResult(HttpResponse.createValueResult args, StatusCode = 200)
        | MediaRemoved _ -> ObjectResult(HttpResponse.createResult(), StatusCode = 200)
        | SubscriptionCreated -> raise(ArgumentException("Unknown event for http mobile response"))
        | SubscriptionRemoved -> raise(ArgumentException("Unknown event for http mobile response"))
        | UserLoggedIn _ -> raise(ArgumentException("Unknown event for http mobile response"))
        | UserLoggedOut _ -> raise(ArgumentException("Unknown event for http mobile response"))
        | ChatMessageSent _ -> raise(ArgumentException("Unknown event for http mobile response"))
        
    let private mapResponse (response: Response) =
        match response with
        | Event args -> ObjectResult(HttpResponse.createValueResult args, StatusCode = 200)
        | Events args -> ObjectResult(HttpResponse.createValueResult args, StatusCode = 200)
        
    let fromEventResult (result: Result<DomainEvent, AppError[]>) =
        match result with
        | Error errors -> mapErrors errors
        | Ok event -> mapEvent event
        
    let fromResponseResult (result: Result<Response, AppError[]>) =
        match result with
        | Error errors -> mapErrors errors
        | Ok response -> mapResponse response
